...
def _node_to_params(node: NodeRecord) -> tuple:
    return (
        node.node_id,
        node.parent_id,
        node.name,
        1 if node.is_directory else 0,
        _dt_to_us(node.ctime),
        _dt_to_us(node.mtime),
        node.mime_type,
        node.hash,
        node.size,
        1 if node.is_image else 0,
        1 if node.is_video else 0,
        node.width,
        node.height,
        node.ms_duration,
    )
