...
from wcpan.drive.feed._lib import dispatch_change
from wcpan.drive.feed._types import MergedChange
...
def node_id_from_change(change: MergedChange) -> str:
    return dispatch_change(
        change, on_removed=lambda node_id: node_id, on_updated=lambda node: node.node_id
    )
