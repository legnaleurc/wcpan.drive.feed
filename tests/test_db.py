...
from ._lib import create_db_sandbox
...
class TestChanges(unittest.TestCase):
    def test_emit_and_get_changes(self):
        with create_db_sandbox() as dsn:
            ...
            self.assertEqual(changes[0].node.node_id, "node-001")
            self.assertIsNotNone(changes[0].node)
    ...
    def test_cursor_advances(self):
        with create_db_sandbox() as dsn:
            ...
            self.assertEqual(len(changes), 1)
            self.assertEqual(changes[0].node.node_id, "node-002")
