...
class TestChanges(unittest.TestCase):
    def test_emit_and_get_changes(self):
        with create_db_sandbox() as dsn:
            ...
            changes, new_cursor, _ = get_changes_since(dsn, 0, 1000)
    ...
    def test_cursor_advances(self):
        with create_db_sandbox() as dsn:
            ...
            changes, new_cursor, _ = get_changes_since(dsn, cursor_after_first, 1000)
            self.assertEqual(len(changes), 1)
    ...
    def test_limit_restricts_rows_and_advances_cursor_partially(self):
        with create_db_sandbox() as dsn:
            ...
            changes, new_cursor, has_more = get_changes_since(dsn, 0, 3)
            self.assertLessEqual(len(changes), 3)
            self.assertGreater(new_cursor, 0)
            self.assertTrue(has_more)

            # Remaining changes are accessible from the new cursor
            remaining, final_cursor, has_more2 = get_changes_since(
                dsn, new_cursor, 1000
            )
    ...
    def test_has_more_true_when_rows_exactly_equal_limit(self):
        with create_db_sandbox() as dsn:
            for i in range(1, 4):
                node = _make_node(f"node-{i:03d}", name=f"file{i}.txt")
                upsert_node(dsn, node)
            ...
            # Limit exactly matches the number of rows — has_more is True
            changes, new_cursor, has_more = get_changes_since(dsn, 0, 3)
            self.assertEqual(len(changes), 3)
            self.assertTrue(has_more)

            # But the next page is empty
            remaining, final_cursor, has_more2 = get_changes_since(dsn, new_cursor, 3)
